...
//go:generate go run go.uber.org/mock/mockgen@latest -destination=mock_servicer_test.go -package=registration -source=handler.go
type Servicer interface {
	Register(ctx context.Context, dto *RegisterDTO) (*RegisterResultDTO, error)
}
...
// parseRegistrationRequestDTO parses and validates an HTTP request to RegisterDTO.
func parseRegistrationRequestDTO(r *http.Request) (*RegisterDTO, error) {
	var req v1.CreateUserRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		return nil, err
	}

	createReq := &RegisterDTO{
		Email:                req.Email,
...
func (h *Handler) Register(w http.ResponseWriter, r *http.Request) {
	req, err := parseRegistrationRequestDTO(r)
	if err != nil {
